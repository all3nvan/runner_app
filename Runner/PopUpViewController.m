...
-(void) presentShareTrailDialog

{
    
    UIAlertController * alert=   [UIAlertController
                                  
                                  alertControllerWithTitle:@"Submit Trail"
                                  
                                  message:@"Name your trail!"
                                  
                                  preferredStyle:UIAlertControllerStyleAlert];
    
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        
        textField.placeholder = @"Trail Name";
        
    }];
    
    
    
    [alert addAction:[UIAlertAction actionWithTitle:@"Save" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        
        
        
        if ([self runDoesNotExistAsTrail])
            
        {
            
            trailSubmission = [[Trail alloc] initWithName:alert.textFields.firstObject.text andRun:self.run andLocations:_locations];
            
            [trailSubmission saveTrailToParse];
...
-(IBAction) saveRun:(id)sender {
    ...
    [_run saveInBackgroundWithBlock:^(BOOL succeeded, NSError * _Nullable error) {
        
        if (trailSubmission != nil && trailSubmission.runID == nil)
            
        {
            
            //trail was named before the run had a chance to save
            
            [trailSubmission updateInformationWithRun:_run];
    ...
    if (self.shareTrail.on)
        
    {
        
        [self presentShareTrailDialog];
